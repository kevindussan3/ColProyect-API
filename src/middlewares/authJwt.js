...
export const isDocente = async(req, res, next) => {
    ...
    return res.status(403).json({ message: "No tienes privilegios docente" })
}
export const isAcudiente = async(req, res, next) => {
    const user = await User.findById(req.userId).then((res) => console.log(res)).watch((error) => console.log(error))
    const roles = await Role.find({ _id: { $in: user.roles } }).then((res) => console.log(res)).watch((error) => console.log(error));
    for (let i = 0; i < roles.length; i++) {
    ...
    return res.status(403).json({ message: "No tienes privilegios acuedinte" })
}